...
module.exports = function(options, issue) {
  ...
  /* Parse requests that request `acdc` as `response_type`.
   *
   * Authorization Code Cross Domain response type depends on an audience
   * indicator and PKCE.  Parsing will error if these parameters are not
   * included, per the specification.  However, the parameters will not actually
   * be parsed.  `oauth2orize-audience` and `oauth2orize-pkce` should be used
   * along side this module for proper parsing and operation.
   *
   * @param {http.ServerRequest} req
   * @api public
   */
  function request(req) {
    if (!req.query.aud) { throw new AuthorizationError('Missing required parameter: aud', 'invalid_request'); }
    if (!req.query.code_challenge) { throw new AuthorizationError('Missing required parameter: code_challenge', 'invalid_request'); }
  }
